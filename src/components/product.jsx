    ...
    <div className="flex flex-col gap-2 w-full md:w-fit font-poppins ">
      <div className="bg-gray-200  rounded-md group relative flex items-center justify-center  md:w-[270px] h-[250px]   overflow-hidden">
        ...
        <img
          src={photo}
          alt={name}
          className="w-[190px] h-[180px] object-contain absolute inset-0 m-auto cursor-pointer"
        />

        {/* Icons */}
        <div className="absolute right-4 top-4 flex flex-col gap-2">
          <div className="bg-white p-1 icon-group rounded-full limit hover:bg-red-400 transition-colors">
            <img src="icons/heart small.svg" alt="heart" className="w-6 h-6 cursor-pointer icon-group-hover:invert " />
          </div>
          <div className="bg-white p-1 rounded-full icon-group hover:bg-red-400 transition-colors">
            <img src="icons/Quick View.svg" alt="view" className="w-6 h-6 cursor-pointer icon-group-hover:invert" />
          </div>
        </div>

        {/* Add To Cart Overlay */}
        <button className="bg-black absolute bottom-0 left-0 w-full py-2 text-center text-white opacity-0 group-hover:opacity-100 transition-opacity">
          Add To Cart
        </button>
      </div>
